...
describe(`parseRange(size, header)`, function () {
  it(`Should create empty "Ranges"`, function () {
    ...
    assert.equal(headerRanges.length, 0);
    assert.equal(headerRanges.type, "");
  });
  ...
  it(`Should parse "header"`, function () {
    ...
    assert.deepStrictEqual(range[0], { start: 0, end: 499 });
  });
  ...
  it(`Should cap end at "size"`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 199 });
  });

  it(`Should parse "header" (40-80)`, function () {
    const range = parseRange(1000, "bytes=40-80");
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 40, end: 80 });
  });
  ...
  it(`Should parse "header" and get plain array`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range.toArray()[0], { start: 40, end: 80 });
    assert.deepStrictEqual(Array.from(range)[0], { start: 40, end: 80 });
    assert.deepStrictEqual([...range][0], { start: 40, end: 80 });
  });
  ...
  it(`Should parse "header" asking for last n bytes`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 600, end: 999 });
  });
  ...
  it(`Should parse "header" with only start`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 400, end: 999 });
  });
  ...
  it(`Should parse "bytes=0-"`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 999 });
  });
  ...
  it(`Should parse "header" with no bytes`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 0 });
  });
  ...
  it(`Should parse "header" asking for last byte`, function () {
    ...
    assert.strictEqual(range.type, "bytes");
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 999, end: 999 });
  });
  ...
  it('should ignore invalid format range when valid range exists', function () {
    ...
    assert.strictEqual(range.type, 'bytes');
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 100, end: 200 });
  });
  ...
  it('should ignore invalid format ranges when some are valid', function () {
    ...
    assert.strictEqual(range.type, 'bytes');
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 100 });
  });
  ...
  it('should ignore invalid format ranges at different positions', function () {
    ...
    assert.deepStrictEqual(range[0], { start: 0, end: 50 });
    assert.deepStrictEqual(range[1], { start: 100, end: 150 });
  });
  ...
  it(`Should parse "header" with multiple ranges`, function () {
    ...
    assert.deepStrictEqual(range[0], { start: 40, end: 80 });
    assert.deepStrictEqual(range[1], { start: 81, end: 90 });
    assert.deepStrictEqual(range[2], { start: 999, end: 999 });
  });
  ...
  it(`Should parse non-byte range`, function () {
    ...
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 5 });
  });
  ...
  describe(`When combine: true`, function () {
    it(`Should combine overlapping ranges`, function () {
      ...
      assert.strictEqual(range.type, "bytes");
      assert.strictEqual(range.length, 2);
      assert.deepStrictEqual(range[0], { start: 0, end: 75 });
      assert.deepStrictEqual(range[1], { start: 90, end: 149 });
    });
    ...
    it(`Should retain original order`, function () {
      ...
      assert.strictEqual(range.type, "bytes");
      assert.strictEqual(range.length, 3);
      assert.deepStrictEqual(range[0], { start: 149, end: 149 });
      assert.deepStrictEqual(range[1], { start: 20, end: 120 });
      assert.deepStrictEqual(range[2], { start: 0, end: 1 });
    });
  });
  ...
  it('should ignore whitespace-only invalid ranges when valid present', function () {
    ...
    assert.strictEqual(range.type, 'bytes');
    assert.strictEqual(range.length, 1);
    assert.deepStrictEqual(range[0], { start: 0, end: 10 });
  });
});
